...
PROFANITY_LIST = [
    "فحش","کس","کیر","کون","جنده","مادرجنده","ننه","بیشعور","خر","گاو","الاغ",
    "احمق","گه","اشغال","بدبخت","بی‌ناموس","ناموس","لاشی","عوضی","آشغال",
    "fuck","shit","bitch","ass","dick","pussy","whore","bastard","idiot"
]
...
async def ai_check_decl(country, text):
    ...
    for bad in PROFANITY_LIST:
        if bad.lower() in text_lower:
            return {"approved":False,"reason":f"بیانیه حاوی کلمه نامناسب است","edited":text}
    # چک طول - حداقل ۱ خط و ۳۰ کاراکتر
    if len(text.strip()) < 30:
        return {"approved":False,"reason":"بیانیه خیلی کوتاه است. حداقل ۳۰ کاراکتر بنویسید","edited":text}
    client = get_ai()
    ...
    try:
        r = client.messages.create(model="claude-sonnet-4-20250514",max_tokens=600,
            system='''تو ناظر رسمی بازی جنگ جهانی ۲۶ هستی. بیانیه رو سختگیرانه بررسی کن.
حتماً رد کن اگه:
- هر نوع فحش، توهین، کلمه رکیک داره (فارسی یا انگلیسی)
- کاملاً بی‌معنی یا چرت و پرت باشه
- غیررسمی باشه
فقط JSON برگردون:
{"approved":true/false,"reason":"دلیل فارسی","edited":"متن اصلاح‌شده"}''',
...
async def ai_war(atk_c,def_c,atk_p,def_p,scenario):
    client = get_ai()
    # لیست دقیق تجهیزات حمله‌کننده
    atk_eq_list = [e for e in atk_p.get('equipment',[]) if e.get('type')=='attack']
    def_eq_list = [e for e in def_p.get('equipment',[]) if e.get('type')=='defense']
    atk_eq = "\n".join(f"- {e['name']}: {e['desc']}" for e in atk_eq_list) or "❌ هیچ تجهیز حمله‌ای ندارد"
    def_eq = "\n".join(f"- {e['name']}: {e['desc']}" for e in def_eq_list) or "❌ هیچ پدافندی ندارد"
    atk_budget = atk_p.get('budget',0)
    def_budget = def_p.get('budget',0)

    if not client:
        has_weapons = len(atk_eq_list) > 0
        return {
            "sat": f"📡 فعالیت نظامی در مرز {atk_c['name']} رصد شد...",
            "winner": "attacker" if has_weapons else "defender",
            "atk_loss": 20 if has_weapons else 80,
            "def_loss": 40 if has_weapons else 5,
            "story": "نبرد رخ داد." if has_weapons else f"{atk_c['name']} بدون تجهیزات حمله کرد و شکست خورد.",
            "territory": "بدون تغییر","civilian":False,"fine":0
        }
    try:
        r = client.messages.create(model="claude-sonnet-4-20250514",max_tokens=1000,
            system='''تو تحلیلگر نظامی دقیق بازی جنگ جهانی ۲۶ هستی.
مهم: نتیجه را کاملاً بر اساس تجهیزات واقعی طرفین تحلیل کن.
- اگه حمله‌کننده تجهیز نداشت، شکست سنگین بخوره
- اگه سناریو با تجهیزات موجود مطابقت نداشت، واقع‌بینانه نتیجه بده
- پدافند قوی = تلفات کمتر مدافع
- موشک بدون لانچر = بی‌اثر
فقط JSON:
{"sat":"خبر ماهواره‌ای هیجان‌انگیز ۲ جمله","winner":"attacker یا defender","atk_loss":عدد_درصد,"def_loss":عدد_درصد,"story":"گزارش دقیق و سینمایی ۴-۵ جمله بر اساس تجهیزات واقعی","territory":"وضعیت ارضی","civilian":true/false,"fine":عدد}''',
            messages=[{"role":"user","content":f"""
⚔️ حمله‌کننده: {atk_c['name']}
💰 بودجه: ${atk_budget:,}
🔫 تجهیزات حمله:
{atk_eq}
...
📋 سناریو ادعایی حمله‌کننده: {scenario}

نکته: اگه سناریو با تجهیزات واقعی مطابقت نداشت یا حمله‌کننده تجهیز کافی نداشت، واقع‌بینانه نتیجه بده.
"""}])
        return json.loads(r.content[0].text.strip().replace("```json","").replace("```",""))
    except:
        return {"sat":"📡 فعالیت نظامی رصد شد...","winner":"defender","atk_loss":30,"def_loss":15,
                "story":"نبرد رخ داد.","territory":"بدون تغییر","civilian":False,"fine":0}
...
async def start(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not p and update.effective_user.id==ADMIN_ID:
        kb=[
            ...
            [InlineKeyboardButton("🌍 انتخاب کشور (اختیاری)",callback_data="choose")],
        ]
        await update.message.reply_text("👑 *خوش اومدی ادمین!*\n\nمیتونی بدون کشور وارد پنل ادمین بشی.",parse_mode="Markdown",reply_markup=InlineKeyboardMarkup(kb))
        return
    ...
    if p:
        ...
        kb=[
            ...
            [InlineKeyboardButton("🌍 وضعیت بازی",callback_data="world"),InlineKeyboardButton("🔗 دعوت دوستان",callback_data="ref")],
        ...
        kb=[
            [InlineKeyboardButton("🌍 انتخاب کشور و ورود",callback_data="choose")],
...
async def tutorial(update:Update,context:ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; await q.answer()
    txt=(
        "📖 *آموزش بازی World War 26*\n"
        "━━━━━━━━━━━━━━━\n\n"
        "🌍 *۱. انتخاب کشور*\n"
        "از لیست کشورها یکی رو انتخاب کن. هر کشور بودجه اولیه، نفت و ویژگی‌های خاص داره.\n\n"
        "🛒 *۲. خرید تجهیزات*\n"
        "با بودجه‌ات تجهیزات بخر:\n"
        "• ✈️ هوایی: جنگنده، بمب‌افکن، پدافند\n"
        "• 🚢 دریایی: ناو، زیردریایی\n"
        "• 🚀 موشکی: انواع موشک و پدافند\n"
        "• 🪖 زمینی: سرباز، تانک، توپخانه\n"
        "• ⛏️ اقتصادی: معدن و پالایشگاه\n"
        "• 🖥️ سایبری: هک و جاسوسی\n\n"
        "📜 *۳. بیانیه رسمی*\n"
        "عکس رییس جمهور یا پرچم بفرست + متن رسمی زیرش بنویس. AI بررسی میکنه و در کانال منتشر میشه.\n\n"
        "⚔️ *۴. جنگ*\n"
        "کشور هدف رو انتخاب کن، سناریو بنویس. AI بر اساس تجهیزات واقعی هر دو طرف نتیجه رو مشخص میکنه!\n\n"
        "💰 *۵. درآمد*\n"
        "هر شب ساعت ۱۲ درآمد معادن و پالایشگاه‌ها واریز میشه.\n\n"
        "📡 *۶. اخبار*\n"
        "هر شب ساعت ۹ BBC خلاصه اتفاقات روز رو در کانال منتشر میکنه.\n\n"
        "🖥️ *۷. هک و جاسوسی*\n"
        "برای هک: باید واحد هک تهاجمی بخری، بعد در سناریو جنگ اعلام هک کن.\n"
        "برای جاسوسی: ماهواره یا سیستم جاسوسی سایبری بخر.\n\n"
        "⚠️ *قوانین مهم*\n"
        "• ۳ روز بیانیه ندی = اخراج از بازی\n"
        "• فحش در بیانیه = رد شدن\n"
        "• سناریو جنگ باید با تجهیزات واقعیت مطابقت داشته باشه\n"
        "• سازمان ملل آسیب به غیرنظامیان رو جریمه میکنه"
    )
...
async def shop_menu(update:Update,context:ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; await q.answer()
    if not GAME_SETTINGS.get("shop_open") and update.effective_user.id!=ADMIN_ID:
        await q.edit_message_text("🔒 فروشگاه توسط ادمین بسته شده!",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="back")]])); return
    db=load_db(); uid=str(update.effective_user.id)
    p=db["players"].get(uid)
    if not p: await q.edit_message_text("❌ /start"); return
    kb=[[InlineKeyboardButton(cat,callback_data=f"cat_{cat}")] for cat in EQUIP]+\
       [[InlineKeyboardButton("📋 لیست کامل تجهیزات",callback_data="fulllist")]]+\
       [[InlineKeyboardButton("🔙 برگشت",callback_data="back")]]
    await q.edit_message_text(f"🛒 *فروشگاه تجهیزات*\n💰 ${p['budget']:,} | 🛢️ {p.get('oil',0):,}",parse_mode="Markdown",reply_markup=InlineKeyboardMarkup(kb))
...
async def decl_start(update:Update,context:ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; await q.answer()
    if not GAME_SETTINGS.get("decl_open",True) and update.effective_user.id!=ADMIN_ID:
        await q.edit_message_text("🔒 بیانیه توسط ادمین بسته شده!",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="back")]])); return ConversationHandler.END
    db=load_db(); uid=str(update.effective_user.id)
    ...
    await q.edit_message_text(
        "📜 *بیانیه رسمی*\n\n"
        "📸 عکس رییس جمهور یا پرچم کشورت رو بفرست\n"
        "✍️ متن بیانیه رو به عنوان *کپشن* (زیر عکس) بنویس\n\n"
        "قوانین:\n"
        "✅ حداقل ۱ خط رسمی\n"
        "✅ بدون فحش و توهین\n"
        "✅ مرتبط با بازی\n\n"
        "⚠️ عکس + متن زیرش رو با هم بفرست!",
        parse_mode="Markdown")
...
async def decl_recv_photo(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.photo:
        await update.message.reply_text("❌ باید عکس بفرستی!\n\nعکس رو با متن زیرش (کپشن) بفرست.")
        return WAITING_DECL_PHOTO

    # گرفتن کپشن
    caption = update.message.caption or ""
    if len(caption.strip()) < 20:
        await update.message.reply_text("❌ متن بیانیه زیر عکس (کپشن) خیلی کوتاهه!\n\nدوباره با متن بیشتر بفرست.")
        return WAITING_DECL_PHOTO
    ...
    if r["approved"]:
        db["declarations"].append({
            "id":len(db["declarations"])+1,
            "user_id":uid,"country":c["name"],
...
async def war_menu(update:Update,context:ContextTypes.DEFAULT_TYPE):
    q=update.callback_query; await q.answer()
    if not GAME_SETTINGS.get("war_open",True) and update.effective_user.id!=ADMIN_ID:
        await q.edit_message_text("🔒 جنگ توسط ادمین بسته شده!",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="back")]])); return
    db=load_db(); uid=str(update.effective_user.id)
    p=db["players"].get(uid)
    if not p: await q.edit_message_text("❌ /start"); return
...
async def war_recv(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    # چک تجهیزات - اگه هیچی نداره نمیتونه حمله کنه
    atk_eq=[e for e in atk_p.get('equipment',[]) if e.get('type')=='attack']
    if not atk_eq:
        await update.message.reply_text(
            "❌ *حمله ممکن نیست!*\n\n"
            "شما هیچ تجهیزات نظامی ندارید!\n"
            "ابتدا از فروشگاه تجهیزات بخرید. 🛒",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛒 فروشگاه",callback_data="shop"),InlineKeyboardButton("🔙 پنل",callback_data="back")]]))
        return ConversationHandler.END

    await update.message.reply_text("⏳ AI داره نبرد رو با دقت تحلیل می‌کنه...")
    ...
    if r.get("civilian") and r.get("fine",0)>0:
        ...
        un=f"\n\n🏛️ *سازمان ملل متحد:*\n{ac['name']} به دلیل آسیب به غیرنظامیان ${fine:,} جریمه شد!"
    save_db(db)
    txt=(
        f"📡 {r.get('sat','')}\n\n"
        f"⚔️ *گزارش کامل نبرد*\n"
...
async def cmd_help(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/removeequip <id> <item_id> - حذف تجهیز\n"
        "/equiplist - لیست ID تجهیزات\n\n"
        "━━ 🔄 ریست ━━\n"
        "/resetplayer <id> - ریست یه پلیر\n"
        "/resetall confirm - ریست همه پلیرا\n\n"
        "━━ ⚙️ کنترل بازی ━━\n"
        "/toggle - نمایش وضعیت بخش‌ها\n"
        "/toggle shop - باز/بستن فروشگاه\n"
        "/toggle decl - باز/بستن بیانیه\n"
        "/toggle war - باز/بستن جنگ\n"
        "/toggle bot - وضعیت کلی بات\n\n"
        "━━ 📢 ارتباط ━━\n"
        "/broadcast <پیام> - پیام به همه\n"
        "/adminstats - آمار کلی بازی\n"
...
async def cmd_toggle(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        await update.message.reply_text(f"⚙️ *وضعیت بازی:*\n{status}\n\nکامندها:\n/toggle shop\n/toggle decl\n/toggle war\n/toggle bot",parse_mode="Markdown"); return
    key=context.args[0].lower()
    mapping={"shop":"shop_open","decl":"decl_open","war":"war_open","bot":"bot_open"}
    if key not in mapping: await update.message.reply_text("❌ گزینه نامعتبر! (shop/decl/war/bot)"); return
    k=mapping[key]
    GAME_SETTINGS[k]=not GAME_SETTINGS.get(k,True)
    status="✅ باز شد" if GAME_SETTINGS[k] else "🔒 بسته شد"
    await update.message.reply_text(f"{status}: {key}")

async def cmd_resetplayer(update:Update,context:ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id!=ADMIN_ID: return
    if not context.args: await update.message.reply_text("استفاده: /resetplayer <user_id>"); return
    db=load_db(); uid=context.args[0]
    if uid not in db["players"]: await update.message.reply_text("❌ پیدا نشد."); return
    c=next((x for x in COUNTRIES if x["id"]==db["players"][uid]["country_id"]),None)
    db["players"][uid]["budget"]=START_BUDGET
    db["players"][uid]["oil"]=c["oil_b"] if c else 0
    db["players"][uid]["equipment"]=[]
    save_db(db)
    await update.message.reply_text(f"🔄 کاربر {uid} ریست شد. بودجه: ${START_BUDGET:,}")
    try: await context.bot.send_message(int(uid),"🔄 دارایی‌های شما توسط ادمین ریست شد.")
    except: pass

async def cmd_resetall(update:Update,context:ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id!=ADMIN_ID: return
    if not context.args or context.args[0]!="confirm":
        await update.message.reply_text("⚠️ این کار همه پلیرا رو ریست میکنه!\nبرای تایید: /resetall confirm"); return
    db=load_db()
    count=0
    for uid,p in db["players"].items():
        c=next((x for x in COUNTRIES if x["id"]==p["country_id"]),None)
        p["budget"]=START_BUDGET
        p["oil"]=c["oil_b"] if c else 0
        p["equipment"]=[]
        count+=1
    save_db(db)
    await update.message.reply_text(f"🔄 {count} پلیر ریست شدند!")

async def cmd_addequip(update:Update,context:ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id!=ADMIN_ID: return
    if len(context.args)<2:
        await update.message.reply_text("استفاده: /addequip <user_id> <item_id>\nمثال: /addequip 123456 atom\nبرای لیست ID ها: /equiplist"); return
    db=load_db(); uid=context.args[0]; iid=context.args[1]
    if uid not in db["players"]: await update.message.reply_text("❌ کاربر پیدا نشد."); return
    item=next((it for cat in EQUIP.values() for it in cat if it["id"]==iid),None)
    if not item: await update.message.reply_text(f"❌ تجهیز '{iid}' پیدا نشد.\nبرای لیست: /equiplist"); return
    db["players"][uid].setdefault("equipment",[]).append(item)
    save_db(db)
    await update.message.reply_text(f"✅ {item['name']} به کاربر {uid} اضافه شد.")
    try: await context.bot.send_message(int(uid),f"🎁 ادمین {item['name']} رو به کشور شما اضافه کرد!")
    except: pass

async def cmd_removeequip(update:Update,context:ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id!=ADMIN_ID: return
    if len(context.args)<2:
        await update.message.reply_text("استفاده: /removeequip <user_id> <item_id>"); return
    db=load_db(); uid=context.args[0]; iid=context.args[1]
    if uid not in db["players"]: await update.message.reply_text("❌ کاربر پیدا نشد."); return
    eq=db["players"][uid].get("equipment",[])
    new_eq=[e for e in eq if e.get("id")!=iid]
    if len(new_eq)==len(eq): await update.message.reply_text("❌ این تجهیز پیدا نشد."); return
    db["players"][uid]["equipment"]=new_eq; save_db(db)
    await update.message.reply_text(f"✅ تجهیز '{iid}' از کاربر {uid} حذف شد.")
...
async def cmd_equiplist(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    for ch in chunks: await update.message.reply_text(ch,parse_mode="Markdown")
...
def main():
    ...
    decl_conv=ConversationHandler(
        ...
        states={
            WAITING_DECL_PHOTO:[
                ...
                MessageHandler(filters.TEXT&~filters.COMMAND,decl_recv),
            ]
        },
    ...
    app.add_handler(CommandHandler("resetplayer",cmd_resetplayer))
    app.add_handler(CommandHandler("resetall",cmd_resetall))
    app.add_handler(CommandHandler("addequip",cmd_addequip))
    app.add_handler(CommandHandler("removeequip",cmd_removeequip))
    app.add_handler(CommandHandler("equiplist",cmd_equiplist))
