...
W_DECL=1;W_WAR=2;W_GIVE_MONEY=3;W_GIVE_OIL=4;W_TRADE_OFFER=5;W_ADM_BUDGET=6;W_ALLIANCE_DECL=7;W_OIL_AMT=8;W_OIL_PRICE=9;W_WARJUDGE=10
...
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer(f"✅ {item['name']} خریداری شد!{ach_txt}", show_alert=True)
...
async def atk_target(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    eq_txt = "\n".join(f"• {e['name']} (💪{e.get('power', 0)})" for e in atk_eq) or "هیچ"
...
async def war_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def_eq = [e for e in def_p.get('equipment', []) if e.get('type') == 'defense']
    if not atk_eq:
    ...
    # بررسی پایه‌ای تجهیزات (بدون AI)
    atk_eq_ids = {e["id"] for e in atk_eq}
    missing = check_scenario(scenario, atk_eq_ids)
    if missing:
        names = ", ".join(get_eq_name(m) for m in missing)
        await update.message.reply_text(
            f"❌ *سناریو تایید نشد!*\n\n"
    ...
    # ثبت جنگ در انتظار بررسی داور
    war_id = str(db.get("war_id_counter", 0) + 1)
    db["war_id_counter"] = int(war_id)
    db.setdefault("pending_wars", {})[war_id] = {
        "atk_uid": uid, "def_uid": tid,
        "ac_name": ac["name"], "dc_name": dc["name"],
        "ac_id": ac["id"], "dc_id": dc["id"],
        "scenario": scenario,
        "atk_eq_names": [e["name"] for e in atk_eq],
        "def_eq_names": [e["name"] for e in def_eq],
    }
    save_db(db)

    atk_eq_txt = "\n".join(f"• {n}" for n in db["pending_wars"][war_id]["atk_eq_names"]) or "هیچ"
    def_eq_txt = "\n".join(f"• {n}" for n in db["pending_wars"][war_id]["def_eq_names"]) or "بدون پدافند"

    try:
        await context.bot.send_message(
            ADMIN_ID,
            f"⚔️ *سناریوی جنگ جدید #{war_id}*\n"
            f"━━━━━━━━━━━━━━━\n"
            f"🔴 حمله‌کننده: {ac['name']}\n"
            f"🔫 تجهیزات:\n{atk_eq_txt}\n\n"
            f"🔵 مدافع: {dc['name']}\n"
            f"🛡️ پدافند:\n{def_eq_txt}\n"
            f"━━━━━━━━━━━━━━━\n"
            f"✍️ *سناریو:*\n{scenario}",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✍️ ثبت نتیجه", callback_data=f"warjudge_{war_id}")]])
        )
    except Exception as e:
    ...
    await update.message.reply_text(
        "✅ *سناریوی شما ثبت شد!*\n\n"
        "⏳ سناریو برای بررسی داور ارسال شد. نتیجه به‌زودی در کانال اعلام می‌شه.",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 پنل", callback_data="back")]])
    )
    return ConversationHandler.END
...
async def warjudge_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    if update.effective_user.id != ADMIN_ID:
        await q.answer("❌", show_alert=True)
        return
    await q.answer()
    war_id = q.data.replace("warjudge_", "")
    db = load_db()
    pending = db.get("pending_wars", {}).get(war_id)
    if not pending:
        await q.edit_message_text("❌ این سناریو دیگه معتبر نیست (شاید قبلاً بررسی شده).")
        return
    kb = [
        [InlineKeyboardButton(f"🔴 برنده: {pending['ac_name']}", callback_data=f"warwin_{war_id}_atk")],
        [InlineKeyboardButton(f"🔵 برنده: {pending['dc_name']}", callback_data=f"warwin_{war_id}_def")],
    ]
    await q.edit_message_text(
        f"⚔️ *سناریو #{war_id}*\n\n"
        f"🔴 {pending['ac_name']} ⚔️ 🔵 {pending['dc_name']}\n\n"
        f"✍️ سناریو:\n{pending['scenario']}\n\n"
        f"برنده این نبرد کیست؟",
        parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb)
    )


async def warjudge_winner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    if update.effective_user.id != ADMIN_ID:
        await q.answer("❌", show_alert=True)
        return ConversationHandler.END
    await q.answer()
    parts = q.data.replace("warwin_", "").rsplit("_", 1)
    war_id, side = parts[0], parts[1]
    db = load_db()
    pending = db.get("pending_wars", {}).get(war_id)
    if not pending:
        await q.edit_message_text("❌ این سناریو دیگه معتبر نیست.")
        return ConversationHandler.END
    context.user_data["warjudge_id"] = war_id
    context.user_data["warjudge_side"] = side
    winner_name = pending["ac_name"] if side == "atk" else pending["dc_name"]
    await q.edit_message_text(
        f"✅ برنده: *{winner_name}*\n\n"
        f"✍️ حالا شرح کامل نتیجه نبرد رو بنویس (همین متن در کانال منتشر می‌شه):\n"
        f"لغو: /cancel",
        parse_mode="Markdown"
    )
    return W_WARJUDGE
...
async def warjudge_text_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db = load_db()
    war_id = context.user_data.get("warjudge_id")
    side = context.user_data.get("warjudge_side")
    pending = db.get("pending_wars", {}).pop(war_id, None) if war_id else None
    if not pending:
        await update.message.reply_text("❌ خطا - سناریو پیدا نشد.")
        return ConversationHandler.END

    result_text = update.message.text
    atk_uid, def_uid = pending["atk_uid"], pending["def_uid"]
    if side == "atk":
        winner_name, loser_name = pending["ac_name"], pending["dc_name"]
        winner_uid, loser_uid = atk_uid, def_uid
    else:
        winner_name, loser_name = pending["dc_name"], pending["ac_name"]
        winner_uid, loser_uid = def_uid, atk_uid

    wp = db["players"].get(winner_uid)
    lp = db["players"].get(loser_uid)
    if wp:
        wp["wars_won"] = wp.get("wars_won", 0) + 1
        db["players"][winner_uid] = wp
    if lp:
        lp["wars_lost"] = lp.get("wars_lost", 0) + 1
        db["players"][loser_uid] = lp

    db["world_tension"] = min(100, db.get("world_tension", 30) + 5)
    db["wars"].append({
        "id": war_id, "atk": pending["ac_name"], "def": pending["dc_name"],
        "atk_uid": atk_uid, "def_uid": def_uid,
        "winner": winner_name, "winner_uid": winner_uid,
        "date": datetime.now().isoformat(),
        "scenario": pending["scenario"][:200],
    })
    save_db(db)

    chan_txt = (
        f"⚔️ *گزارش نبرد #{war_id}*\n"
        f"━━━━━━━━━━━━━━━\n"
        f"🔴 {pending['ac_name']}  ⚔️  🔵 {pending['dc_name']}\n\n"
        f"✍️ *سناریو:*\n{pending['scenario']}\n\n"
        f"📰 *شرح نتیجه:*\n{result_text}\n\n"
    ...
    try:
        ...
        logger.error(f"warjudge channel error: {e}")

    for u in (atk_uid, def_uid):
        try:
            await context.bot.send_message(int(u),
                f"⚔️ *نتیجه نبرد اعلام شد!*\n\n🏆 برنده: {winner_name}\n\n📰 {result_text}",
                parse_mode="Markdown")
        except:
            pass

    await update.message.reply_text("✅ نتیجه ثبت و در کانال منتشر شد!")
    return ConversationHandler.END
...
async def trade_oil_sell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """نمایش لیست بازیکنان برای فروش نفت دستی"""
    q = update.callback_query
    await q.answer()
    db = load_db()
    uid = str(update.effective_user.id)
    ...
    if not p:
        return
    if p.get('oil', 0) < 1:
        await q.edit_message_text("❌ نفتی برای فروش نداری!",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="trade_menu")]]))
        return
    others = {k: v for k, v in db["players"].items() if k != uid and not v.get("banned")}
    if not others:
        await q.edit_message_text("❌ بازیکن دیگه‌ای نیست!",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="trade_menu")]]))
        return
    kb = []
    for k, v in others.items():
        ...
        if c:
            kb.append([InlineKeyboardButton(f"🛢️ فروش به {c['name']}", callback_data=f"oilt_target_{k}")])
    kb.append([InlineKeyboardButton("🔙", callback_data="trade_menu")])
    await q.edit_message_text(
        f"🛢️ *فروش نفت*\n"
        f"موجودی شما: {p.get('oil', 0):,} بشکه\n\n"
        f"به کدوم کشور می‌فروشی؟",
        parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb)
    )


async def oilt_target(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    tid = q.data.replace("oilt_target_", "")
    db = load_db()
    uid = str(update.effective_user.id)
    p = db["players"].get(uid)
    ...
    context.user_data["oil_target"] = tid
    await q.edit_message_text(
        f"🛢️ *فروش نفت به {tc['name'] if tc else '?'}*\n\n"
        f"موجودی شما: {p.get('oil', 0):,} بشکه\n\n"
        f"✍️ چند بشکه می‌فروشی؟ (فقط عدد بفرست)\n"
        f"لغو: /cancel",
        parse_mode="Markdown"
    )
    return W_OIL_AMT
...
async def oilt_amt_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db = load_db()
    uid = str(update.effective_user.id)
    p = db["players"].get(uid)
    if not p:
    ...
    try:
        ...
        await update.message.reply_text("❌ فقط عدد بفرست! مثلاً: 5000")
        return W_OIL_AMT
    if amount <= 0:
        await update.message.reply_text("❌ عدد باید بیشتر از صفر باشه!")
        return W_OIL_AMT
    if amount > p.get("oil", 0):
        await update.message.reply_text(f"❌ نفت کافی نداری! موجودی: {p.get('oil',0):,} بشکه")
        return W_OIL_AMT
    context.user_data["oil_amount"] = amount
    await update.message.reply_text(
        f"🛢️ {amount:,} بشکه\n\n"
        f"✍️ کل قیمت (به دلار) چنده؟ (فقط عدد بفرست)\n"
        f"لغو: /cancel"
    )
    return W_OIL_PRICE


async def oilt_price_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db = load_db()
    uid = str(update.effective_user.id)
    p = db["players"].get(uid)
    tid = context.user_data.get("oil_target")
    tp = db["players"].get(tid)
    amount = context.user_data.get("oil_amount")
    if not p or not tp or not amount:
        return ConversationHandler.END
    try:
        price = int(update.message.text.replace(",", "").replace("$", "").strip())
    except:
        await update.message.reply_text("❌ فقط عدد بفرست! مثلاً: 50000")
        return W_OIL_PRICE
    if price <= 0:
        await update.message.reply_text("❌ قیمت باید بیشتر از صفر باشه!")
        return W_OIL_PRICE

    ac = get_country(p.get("country_id", ""))
    tc = get_country(tp.get("country_id", ""))
    offer_id = str(db.get("oil_offer_counter", 0) + 1)
    db["oil_offer_counter"] = int(offer_id)
    db.setdefault("oil_offers", {})[offer_id] = {
        "seller": uid, "buyer": tid, "amount": amount, "price": price, "status": "pending"
    }
    save_db(db)

    kb = [[InlineKeyboardButton("✅ قبول", callback_data=f"oiloffer_accept_{offer_id}"),
           InlineKeyboardButton("❌ رد", callback_data=f"oiloffer_reject_{offer_id}")]]
    try:
        await context.bot.send_message(
            int(tid),
            f"🛢️ *پیشنهاد خرید نفت*\n\n"
            f"از: {ac['name'] if ac else '?'}\n"
            f"🛢️ مقدار: {amount:,} بشکه\n"
            f"💰 قیمت کل: ${price:,}\n\n"
            f"موجودی شما: ${tp.get('budget',0):,}",
            parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb)
        )
    except:
        pass
    ...
    await update.message.reply_text(
        f"✅ *پیشنهاد ارسال شد!*\n\n"
        f"به: {tc['name'] if tc else '?'}\n"
        f"🛢️ {amount:,} بشکه → 💰 ${price:,}\n\n"
        f"منتظر پاسخ بمون.",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 پنل", callback_data="back")]])
    )
...
async def oiloffer_accept(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    offer_id = q.data.replace("oiloffer_accept_", "")
    db = load_db()
    offer = db.get("oil_offers", {}).get(offer_id)
    uid = str(update.effective_user.id)
    if not offer or offer.get("status") != "pending":
        await q.answer("❌ این پیشنهاد دیگه معتبر نیست!", show_alert=True)
        return
    if offer["buyer"] != uid:
        await q.answer("❌ این پیشنهاد برای شما نیست!", show_alert=True)
        return
    seller = db["players"].get(offer["seller"])
    buyer = db["players"].get(offer["buyer"])
    if not seller or not buyer:
        await q.answer("❌ خطا!", show_alert=True)
        return
    if buyer.get("budget", 0) < offer["price"]:
        await q.answer(f"❌ بودجه کافی نداری! نیاز: ${offer['price']:,}", show_alert=True)
        return
    if seller.get("oil", 0) < offer["amount"]:
        await q.answer("❌ فروشنده دیگه این مقدار نفت رو ندارد!", show_alert=True)
        offer["status"] = "rejected"
        save_db(db)
        return

    seller["oil"] = seller.get("oil", 0) - offer["amount"]
    seller["budget"] = seller.get("budget", 0) + offer["price"]
    buyer["oil"] = buyer.get("oil", 0) + offer["amount"]
    buyer["budget"] = buyer.get("budget", 0) - offer["price"]
    db["players"][offer["seller"]] = seller
    db["players"][offer["buyer"]] = buyer
    offer["status"] = "accepted"
    save_db(db)

    sc = get_country(seller.get("country_id", ""))
    bc = get_country(buyer.get("country_id", ""))
    try:
        await context.bot.send_message(int(offer["seller"]),
            f"✅ *{bc['name'] if bc else '?'} پیشنهادتو قبول کرد!*\n"
            f"🛢️ {offer['amount']:,} بشکه فروخته شد\n"
            f"💰 +${offer['price']:,}", parse_mode="Markdown")
    except:
        pass
    await q.edit_message_text(
        f"✅ *معامله انجام شد!*\n\n"
        f"از {sc['name'] if sc else '?'}\n"
        f"🛢️ {offer['amount']:,} بشکه دریافت شد\n"
        f"💰 -${offer['price']:,}", parse_mode="Markdown"
    )


async def oiloffer_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    offer_id = q.data.replace("oiloffer_reject_", "")
    db = load_db()
    offer = db.get("oil_offers", {}).get(offer_id)
    uid = str(update.effective_user.id)
    if not offer or offer.get("status") != "pending":
    ...
    offer["status"] = "rejected"
    save_db(db)
    seller_p = db["players"].get(offer["seller"])
    buyer_p = db["players"].get(offer["buyer"])
    bc = get_country(buyer_p.get("country_id", "")) if buyer_p else None
    try:
        await context.bot.send_message(int(offer["seller"]),
            f"❌ {bc['name'] if bc else '?'} پیشنهاد فروش نفت شما را رد کرد.")
    except:
        pass
    await q.edit_message_text("❌ پیشنهاد رد شد.")
...
async def send_update_news(app, version=CURRENT_VERSION):
    """ارسال پیام آپدیت به همه بازیکنان و کانال - فقط یک بار برای هر نسخه"""
    db = load_db()
    if db.get("last_update_version") == version:
        return
    msg = (
        ...
        "🆕 *چیزای جدید:*\n\n"
        "🛒 *فروشگاه:*\n"
        "• با هر خرید پیام تایید نشون داده می‌شه\n\n"
        "🛢️ *معامله نفت جدید:*\n"
        "• کشور مقصد، تعداد بشکه و قیمت رو خودت دستی تعیین می‌کنی\n"
        "• طرف مقابل پیشنهادتو قبول/رد می‌کنه\n\n"
        "⚔️ *جنگ - سیستم جدید داوری:*\n"
        "• سناریوی حمله رو می‌نویسی\n"
        "• نتیجه نبرد رو داور بررسی و اعلام می‌کنه\n"
        "• نتیجه در کانال منتشر و به هر دو طرف اطلاع داده می‌شه\n\n"
        "📜 *بیانیه:*\n"
        "• فقط فحش رد می‌شه، بقیه بیانیه‌های رسمی تایید می‌شن\n\n"
        "🚪 *دکمه خروج از بازی* اضافه شد\n\n"
        f"📢 {CHANNEL_LINK}\n"
        "برای شروع: /start"
    )
    count = 0
    ...
    db["last_update_version"] = version
    save_db(db)
...
async def al_leave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    aid = q.data.replace("al_leave_","")
    db = load_db()
    uid = str(update.effective_user.id)
    al = db.get("alliances",{}).get(aid)
    if not al: return
    if uid in al.get("members",[]):
        al["members"].remove(uid)
        save_db(db)
        c = get_country(db["players"].get(uid,{}).get("country_id",""))
        try: await context.bot.send_message(int(al["leader"]), f"⚠️ {c['name'] if c else '?'} از اتحاد {al['name']} خارج شد.")
        except: pass
    await q.edit_message_text(f"✅ از اتحاد {al['name']} خارج شدی.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="back")]]))

async def al_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    parts = q.data.replace("al_reject_","").split("_")
    req_uid = parts[-1]
    aid = "_".join(parts[:-1])
    db = load_db()
    al = db.get("alliances",{}).get(aid)
    if not al: return
    if req_uid in al.get("requests",[]):
        al["requests"].remove(req_uid)
        save_db(db)
    try: await context.bot.send_message(int(req_uid), f"❌ درخواست عضویت شما در اتحاد {al['name']} رد شد.")
    except: pass
...
def main():
    ...
    conv_oiltrade = ConversationHandler(
        entry_points=[CallbackQueryHandler(oilt_target, pattern="^oilt_target_")],
        states={
            W_OIL_AMT:[MessageHandler(filters.TEXT & ~filters.COMMAND, oilt_amt_recv), CommandHandler("cancel", cancel)],
            W_OIL_PRICE:[MessageHandler(filters.TEXT & ~filters.COMMAND, oilt_price_recv), CommandHandler("cancel", cancel)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )
    conv_warjudge = ConversationHandler(
        entry_points=[CallbackQueryHandler(warjudge_winner, pattern="^warwin_")],
        states={W_WARJUDGE:[
            MessageHandler(filters.TEXT & ~filters.COMMAND, warjudge_text_recv),
            CommandHandler("cancel", cancel),
        ]},
        fallbacks=[CommandHandler("cancel", cancel)],
    )
    ...
    app.add_handler(conv_oiltrade)
    app.add_handler(conv_warjudge)
    app.add_handler(CallbackQueryHandler(warjudge_start, pattern="^warjudge_"))
    ...
    app.add_handler(CallbackQueryHandler(oiloffer_accept, pattern="^oiloffer_accept_"))
    app.add_handler(CallbackQueryHandler(oiloffer_reject, pattern="^oiloffer_reject_"))
    ...
    app.add_handler(CallbackQueryHandler(al_leave, pattern="^al_leave_"))
    app.add_handler(CallbackQueryHandler(al_reject, pattern="^al_reject_"))
