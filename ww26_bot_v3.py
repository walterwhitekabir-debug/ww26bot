...
async def ai_war(atk_c,def_c,atk_p,def_p,scenario):
    ...
    try:
        r=client.messages.create(model="claude-sonnet-4-20250514",max_tokens=1200,
            system='''تحلیلگر نظامی دقیق بازی جنگ جهانی ۲۶.
قوانین:
- فقط از تجهیزات لیست‌شده استفاده کن
- اگه سناریو با تجهیزات تناقض داشت، واقعیت رو بنویس
- اشغال فقط با نیروی زمینی (infantry/special/marine/t72/abrams/para)
- گزارش با اسم شهرها و تعداد دقیق تلفات
- اگه بمب اتم: فاجعه کامل رو بنویس
JSON: {"sat":"خبر ماهواره ۲ جمله","winner":"attacker یا defender","atk_loss":عدد,"def_loss":عدد,"story":"گزارش کامل ۵-۷ جمله","territory":"وضعیت ارضی","civilian":true/false,"fine":عدد,"occupied":true/false}''',
...
async def decl_recv_photo(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if r["approved"]:
        ...
        ch_cap=f"📜 *بیانیه رسمی*\n🌍 {c['name']}\n━━━━━━━━━━━━━━━\n{r['edited']}"
        try:
            await context.bot.send_photo(CHANNEL_ID,photo=update.message.photo[-1].file_id,caption=ch_cap,parse_mode="Markdown")
...
async def trade_accept(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    try: await context.bot.send_message(int(trade["from_uid"]),f"✅ *معامله تایید شد!*\n{trade['to_country']} پذیرفت.\n📋 {trade['detail']}")
    except: pass
    if not trade.get("secret"):
        try: await context.bot.send_message(CHANNEL_ID,f"🤝 *معامله رسمی*\n━━━━━━━━━━━━━━━\n🔵 {trade['from_country']} → 🟢 {trade['to_country']}\n{'🚛' if trade['method']=='land' else '🚢'} {trade['detail']}\n🗺️ {trade['route']}",parse_mode="Markdown")
        except: pass
    await q.edit_message_text(f"✅ معامله با {trade['from_country']} تایید شد!")
...
async def cmd_help(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/givemoney \<مقدار\> - پول به همه بازیکنان\n"
        "/addequip \<id\> \<item\_id\> - اضافه کردن تجهیز\n"
        "/removeequip \<id\> \<item\_id\> - حذف یک تجهیز\n"
        "/clearequip \<id\> - حذف همه تجهیزات پلیر\n"
        "/equiplist - لیست ID تجهیزات\n\n"
        "━━ 🌍 مدیریت کشورها ━━\n"
        "/listcountries - لیست کشورها با ID\n"
        "/setcountry \<id\> \<country\_id\> - تغییر کشور پلیر\n"
        "/occupiedlist - لیست کشورهای اشغالی\n"
        "/freecountry \<country\_id\> - آزاد کردن یک کشور\n"
        "/freeall - آزاد کردن همه کشورهای اشغالی\n\n"
        "━━ 🔄 ریست ━━\n"
        ...
        "/toggle trade - باز/بستن معامله\n\n"
        "━━ 📜 آمار و تاریخچه ━━\n"
        "/adminstats - آمار کلی بازی\n"
        "/trades - آخرین معاملات\n"
        "/clearwars confirm - پاک کردن تاریخچه جنگ\n"
        "/cleardecls confirm - پاک کردن بیانیه‌ها\n\n"
        "━━ 📢 ارتباط ━━\n"
        "/broadcast \<پیام\> - پیام به همه بازیکنان\n"
